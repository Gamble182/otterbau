...
type Props = { from?: string; to?: string; topN?: number };

const palette = ["#60a5fa","#34d399","#f472b6","#f59e0b","#a78bfa","#fb7185","#22d3ee","#c084fc","#4ade80","#fca5a5"];
...
export default function ExpensesByCategoryPie({ from, to, topN = 8 }: Props) {
  ...
  const data = useMemo(() => {
    ...
    if (arr.length > topN) {
      ...
      if (tail > 0) head.push({ name: "Sonstige", value: tail });
      arr = head;
    }
    return arr;
  }, [items, from, to, topN]);
        ...
        <ResponsiveContainer>
          <PieChart>
            <Pie data={data} dataKey="value" nameKey="name" innerRadius={70} outerRadius={120} paddingAngle={2}>
              ...
              <Label value={`${total.toFixed(2)} €`} position="center" className="text-base" />
            </Pie>
